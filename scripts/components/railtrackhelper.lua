...
local function IsCaveVoidTrack(inst)
    return inst and inst.player_classified and inst.player_classified.railtrack_tool_iscavevoid:value()
end
...
function RailTrackHelper:OnUpdate(dt)
    ...
    if IsValid(self.ring_pos) then
        if #ent == 0 then
            ...
            local var =
                TheSim:FindEntities(tpos.x - dx, 0, tpos.z - dz, 0.9 * self.track_length, {"railtrack"}, {"burnt"})
            local aux =
                TheSim:FindEntities(tpos.x - dx, 0, tpos.z - dz, 0.9 * self.track_length, {"railtrack", "burnt"})
        ...
        local iscavevoidtrack = IsCaveVoidTrack(self.inst)
        local isovercavevoid = IsOverCaveVoid(pos)

        if isovercavevoid then
            -- Over cave void - only cavevoid track allowed
            if not iscavevoidtrack then
                self.ring_pos.AnimState:SetAddColour(0.5, 0.0, 0, 0)
            elseif not self.isred then
                self.ring_pos.AnimState:SetAddColour(0, 0.5, 0, 0)
            end
        elseif IsOverWater(pos) then
            -- Over water - only ocean track allowed
            ...
            -- On normal ground - ocean and cavevoid tracks not allowed
            if isoceantrack or iscavevoidtrack then
                self.ring_pos.AnimState:SetAddColour(0.5, 0.0, 0, 0)
            elseif not self.isred then
                self.ring_pos.AnimState:SetAddColour(0, 0.5, 0, 0)
            end
