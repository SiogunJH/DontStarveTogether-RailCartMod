...
local function OnLoad(inst, data)
    if
        data ~= nil and data.burnt and inst.components.burnable ~= nil and inst.components.burnable.onburnt ~= nil and
            not inst.is_fireproof
     then
...
local function CaveVoidPlace(inst)
    local point = inst:GetPosition()
    -- Cavevoid rails can only be placed on impassable tiles (cave void)
    if not TheWorld.Map:IsImpassableTileAtPoint(point.x, point.y, point.z) then
        OnHammered(inst, inst)
        return
    end
end
        ...
        if placefn ~= nil then
            inst:DoTaskInTime(
                ...
                function()
                    ...
                    local pos = inst:GetPosition()
                    if
                        inst ~= nil and inst:IsValid() and
                            (inst:GetCurrentPlatform() ~= nil or
                                TheWorld.Map:GetTileAtPoint(inst:GetPosition():Get()) == 1 or
                                #TheSim:FindEntities(
                                    pos.x,
                                    0,
                                    pos.z,
                                    0.9 * inst.components.railtrack.track_length,
                                    {"railtrack"}
                                ) > 1)
                     then
                        OnHammered(inst, inst)
...
return create_railtrack(
    ...
    assets,
    prefabs
), create_railtrack_item(
    "railtrack_default",
    "railtrack",
    "railtrack",
    "railtrack_default",
    "images/inventoryimages/railtrack.xml",
    "default",
    assets,
    prefabs
), create_railtrack(
